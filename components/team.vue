<template>
  <div id="team">
    <div class="text-center py-24">
      <p class="text-5xl gibsonbold">Team and Experience</p>
      <p class="text-lg text-gray-700 gibson">30+ YEARS COMBINED OF WEB2 GAMING</p>
      <img
        src="~/assets/image/section_line.png"
        alt="Line"
        class="m-auto my-2"
      />
      <div class="grid grid-cols-4 gap-x-16 gap-y-0 mx-0 sm:mx-56 my-16 gibson">
        <div class="col-span-4 sm:col-span-1 mx-12 sm:mx-0">
          <div class="photo-wrapper">
            <img
              src="~/assets/image/team/team1.jpg"
              class="w-full h-full"
              alt=""
            />
          </div>
          <p class="text-2xl mt-4">SAMET PALA<font-awesome-icon icon="fab fa-linkedin-in" class="mx-2 bg-sky-600 text-white text-base rounded p-1" /></p>
          <p class="text-lg">CEO OF THE WAKE</p>
        </div>

        <div class="col-span-4 sm:col-span-1 mx-12 sm:mx-0">
          <div class="photo-wrapper">
            <img
              src="~/assets/image/team/team2.jpg"
              class="w-full h-full"
              alt=""
            />
          </div>
          <p class="text-2xl mt-4">SAMET PALA<font-awesome-icon icon="fab fa-linkedin-in" class="mx-2 bg-sky-600 text-white text-base rounded p-1" /></p>
          <p class="text-lg">CEO OF THE WAKE</p>
        </div>

        <div class="col-span-4 sm:col-span-1 mx-12 sm:mx-0">
          <div class="photo-wrapper">
            <img
              src="~/assets/image/team/team3.jpg"
              class="w-full h-full"
              alt=""
            />
          </div>
          <p class="text-2xl mt-4">SAMET PALA<font-awesome-icon icon="fab fa-linkedin-in" class="mx-2 bg-sky-600 text-white text-base rounded p-1" /></p>
          <p class="text-lg">CEO OF THE WAKE</p>
        </div>

        <div class="col-span-4 sm:col-span-1 mx-12 sm:mx-0">
          <div class="photo-wrapper">
            <img
              src="~/assets/image/team/team4.jpg"
              class="w-full h-full"
              alt=""
            />
          </div>
          <p class="text-2xl mt-4">SAMET PALA<font-awesome-icon icon="fab fa-linkedin-in" class="mx-2 bg-sky-600 text-white text-base rounded p-1" /></p>
          <p class="text-lg">CEO OF THE WAKE</p>
        </div>

      </div>

      <div class="grid grid-cols-5 gap-x-16 gap-y-0 mx-0 sm:mx-24 my-16">
        <div class="col-span-5 sm:col-span-1 mx-12 sm:mx-0">
          <div class="photo-wrapper">
            <img
              src="~/assets/image/team/team5.jpg"
              class="w-full h-full"
              alt=""
            />
          </div>
          <p class="text-2xl mt-4">SAMET PALA<font-awesome-icon icon="fab fa-linkedin-in" class="mx-2 bg-sky-600 text-white text-base rounded p-1" /></p>
          <p class="text-lg">CEO OF THE WAKE</p>
        </div>

        <div class="col-span-5 sm:col-span-1 mx-12 sm:mx-0">
          <div class="photo-wrapper">
            <img
              src="~/assets/image/team/team6.jpg"
              class="w-full h-full"
              alt=""
            />
          </div>
          <p class="text-2xl mt-4">SAMET PALA<font-awesome-icon icon="fab fa-linkedin-in" class="mx-2 bg-sky-600 text-white text-base rounded p-1" /></p>
          <p class="text-lg">CEO OF THE WAKE</p>
        </div>

        <div class="col-span-5 sm:col-span-1 mx-12 sm:mx-0">
          <div class="photo-wrapper">
            <img
              src="~/assets/image/team/team7.jpg"
              class="w-full h-full"
              alt=""
            />
          </div>
          <p class="text-2xl mt-4">SAMET PALA<font-awesome-icon icon="fab fa-linkedin-in" class="mx-2 bg-sky-600 text-white text-base rounded p-1" /></p>
          <p class="text-lg">CEO OF THE WAKE</p>
        </div>

        <div class="col-span-5 sm:col-span-1 mx-12 sm:mx-0">
          <div class="photo-wrapper">
            <img
              src="~/assets/image/team/team8.jpg"
              class="w-full h-full"
              alt=""
            />
          </div>
          <p class="text-2xl mt-4">SAMET PALA<font-awesome-icon icon="fab fa-linkedin-in" class="mx-2 bg-sky-600 text-white text-base rounded p-1" /></p>
          <p class="text-lg">CEO OF THE WAKE</p>
        </div>

        <div class="col-span-5 sm:col-span-1 mx-12 sm:mx-0">
          <div class="photo-wrapper">
            <img
              src="~/assets/image/team/team9.jpg"
              class="w-full h-full"
              alt=""
            />
          </div>
          <p class="text-2xl mt-4">SAMET PALA<font-awesome-icon icon="fab fa-linkedin-in" class="mx-2 bg-sky-600 text-white text-base rounded p-1" /></p>
          <p class="text-lg">CEO OF THE WAKE</p>
        </div>
      </div>
    </div>
  </div>
</template>
